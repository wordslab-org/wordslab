...
sudo -- bash -c 'git clone https://github.com/jupyter/docker-stacks.git /var/cache/docker-stacks'
...
sudo -- bash -c 'apt-get update --yes && DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends locales tini wget python3.10-venv && apt-get clean && rm -rf /var/lib/apt/lists/*'

# Configure locale
sudo -- bash -c 'echo "en_US.UTF-8 UTF-8" > /etc/locale.gen && locale-gen'
...
# Install all OS dependencies for notebook server that starts but lacks all
# features (e.g., download as all possible file formats)
sudo -- bash -c 'apt-get update --yes && DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends fonts-liberation pandoc run-one && apt-get clean && rm -rf /var/lib/apt/lists/*'

# JupyterLab source extensions require Node.js to rebuild JupyterLab and activate the extension
export NODE_MAJOR=18
sudo -- bash -c 'apt-get update --yes && apt-get install -y ca-certificates curl gnupg && mkdir -p /etc/apt/keyrings && curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | gpg --dearmor -o /etc/apt/keyrings/nodesource.gpg'
sudo -- bash -c 'export NODE_MAJOR=18 && echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_$NODE_MAJOR.x nodistro main" | sudo tee /etc/apt/sources.list.d/nodesource.list'
sudo -- bash -c 'apt-get update && apt-get install nodejs -y && rm -rf /var/lib/apt/lists/*'
...
sudo -- bash -c 'pip install notebook jupyterhub jupyterlab==3.6.5 ipywidgets &&  jupyter notebook --generate-config && npm cache clean --force && jupyter lab clean && rm -rf "${HOME}/.cache/yarn"'
